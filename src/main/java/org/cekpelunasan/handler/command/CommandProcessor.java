...
public interface CommandProcessor {
	...
	default void sendDocument(Long chatId, String text, InputFile inputFile, TelegramClient telegramClient) {
		// Convert InputFile to bytes may not be trivial; keep existing path for
		// captionless doc via service
		// If caption is needed, prepend text into a separate message
		// Here we fallback to Telegram client only for InputFile variant if necessary
		// in future
